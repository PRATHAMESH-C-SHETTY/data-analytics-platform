...
#  CUSTOM CLEANING API
# ══════════════════════════════════════════════════════════════
@app.route('/api/clean/<int:dataset_id>', methods=['POST'])
def api_clean(dataset_id):
    """Custom data cleaning with user-specified options."""
    dataset = db.session.get(Dataset, dataset_id)
    if not dataset:
        return jsonify({'error': 'Dataset not found'}), 404
    ...
    try:
        ...
        # Load and clean data with custom options
        df = load_dataset(dataset.file_path, dataset.file_type)
        df_clean, cleaning_report = clean_dataset(df, options=options)
